
// src/assets/images/index.ts
export const Images = {
    background: require('./background.png'),
    logo: require('./logo.png'),
    googleIcon: require('./googleIcon.png'),
    appleIcon: require('./appleIcon.png'),
    arrowLeftLight: require('./arrowLeftLight.png'),
    logoText: require('./logoText.png'),
    closeIcon: require('./closeIcon.png'),
    faceIdIcon: require('./faceIdIcon.png'),
    dashboard: require('./dashboard.png'),
    dashboardActive: require('./dashboardActive.png'),
    accounts: require('./accounts.png'),
    accountsActive: require('./accountsActive.png'),
    scoutAI: require('./scoutAI.png'),
    scoutAIActive: require('./scoutAIActive.png'),
    generator: require('./generator.png'),
    generatorActive: require('./generatorActive.png'),
    vault: require('./vault.png'),
    vaultActive: require('./vaultActive.png'),
    burgerMenu: require('./burgerMenu.png'),
    noNotification: require('./noNotificationIcon.png'),
    notifications: require('./notificationsIcon.png'),
    plusIcon: require('./plusIcon.png')

};
